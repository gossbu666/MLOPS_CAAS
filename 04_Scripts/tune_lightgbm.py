"""
CAAS — LightGBM Optuna Tuning (Phase B of SP7)

Parallel tree model to XGBoost. Same split, same features, comparable search.

Usage:
    python 04_Scripts/tune_lightgbm.py                        # 100 trials per horizon
    python 04_Scripts/tune_lightgbm.py --n-trials 3 --smoke   # quick mechanism test
    python 04_Scripts/tune_lightgbm.py --horizons t7          # single horizon

Requirements: features.csv must exist.
Studies resume from SQLite at 03_Data/results/optuna_studies/lgbm_<horizon>.db.
"""
...
def make_objective(X_train, y_train, X_val, y_val, horizon):
    ...
    def objective(trial: optuna.Trial):
        params = {
            ...
            "objective":         "regression_l1",   # aligns with MAE objective
            "metric":            "mae",
...
def tune_one_horizon(horizon, n_trials, df_train, df_val, df_test, feature_cols):
    ...
    print(f"\n{'=' * 60}")
    print(f"🎯  Tuning LightGBM for horizon {horizon.upper()}")
    ...
    mlflow.set_experiment("CAAS-LGBM-Optuna")
    with mlflow.start_run(run_name=f"lgbm_tune_{horizon}"):
        ...
        final_model.fit(X_full_f, y_full)
        ...
        # Save model (LightGBM uses booster.save_model to a .txt)
        model_path = MODELS_DIR / f"lightgbm_{horizon}.txt"
        ...
        pred_path = RESULTS_DIR / f"lightgbm_{horizon}_test_predictions.csv"
        ...
        imp_path = RESULTS_DIR / f"lightgbm_{horizon}_importance.csv"
...
def main():
    ...
    if args.smoke:
        print("🧪  SMOKE mode — compact study DBs, champion summary not overwritten")
    ...
    if not args.smoke:
        summary_path = RESULTS_DIR / "lightgbm_summary.json"
